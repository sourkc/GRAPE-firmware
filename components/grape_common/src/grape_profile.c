...
static const char *const s_metric_names[GRAPE_PROFILE_METRIC_COUNT] = {
    ...
    [GRAPE_PROFILE_METRIC_SHEAR_PREP] = "shear prep",
    [GRAPE_PROFILE_METRIC_SHEAR_X1] = "shear X pass 1",
    [GRAPE_PROFILE_METRIC_SHEAR_Y] = "shear Y pass",
    [GRAPE_PROFILE_METRIC_SHEAR_X2] = "shear X pass 2",
...
{
    if ((unsigned)metric >= GRAPE_PROFILE_METRIC_COUNT || !s_metric_names[metric]) {
        return "unknown";
    }
...
void grape_profile_report_if_due(void)
{
    ...
    for (int i = 0; i < GRAPE_PROFILE_METRIC_COUNT; ++i) {
        ...
        ESP_LOGI(TAG,
                 ...
                 grape_profile_metric_name((grape_profile_metric_t)i),
                 total_ms,
                 avg_ms,
                 max_ms,
                 stat->calls,
...
void grape_profile_snapshot(grape_profile_snapshot_t *out_snapshot)
{
    ...
    memcpy(out_snapshot->metrics, s_stats, sizeof(s_stats));
}
