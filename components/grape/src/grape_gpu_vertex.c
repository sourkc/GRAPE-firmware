...
static esp_err_t gpu_fetch_position(const grape_gpu_pipeline_t *pipeline,
                               ...
                               grape_gpu_clip_vertex_t *output)
{
    ...
    *output = (grape_gpu_clip_vertex_t) {
                                           ...
                                           grape_gpu_clip_vertex_t *out_vertex)
{
    ...
    esp_err_t ret = gpu_fetch_position(
...
static grape_gpu_clip_vertex_t gpu_clip_lerp(const grape_gpu_clip_vertex_t *a,
                                              ...
                                              float t)
{
    return (grape_gpu_clip_vertex_t) {
        .x = a->x + (b->x - a->x) * t,
        .y = a->y + (b->y - a->y) * t,
        .z = a->z + (b->z - a->z) * t,
        .w = a->w + (b->w - a->w) * t,
                                 ...
                                 grape_gpu_clip_vertex_t output[GRAPE_GPU_MAX_CLIPPED_VERTICES])
{
    ...
    for (gpu_clip_plane_t plane = GPU_CLIP_LEFT;
        ...
        for (uint32_t i = 0U; i < input_count; ++i) {
            ...
            float current_distance = gpu_clip_distance(&current, plane);
                               ...
                               const grape_gpu_clip_vertex_t triangle[3])
{
    ...
    for (uint32_t i = 0U; i < 3U; ++i) {
        ...
        float inverse_w = 1.0f / triangle[i].w;
    ...
    bool front_facing = pipeline->desc.front_face == GRAPE_GPU_FRONT_FACE_CCW
        ? signed_area > 0.0f
        : signed_area < 0.0f;
